...
;{
  let debugMode = true,
    log = str => window.console && console.log(str)
  
  let pick = (data) => {
    data = Object.prototype.toString.apply(data) !== '[object Object]' ? {} : data
    ...
    function check (...args) {
      let i = 0,
        ...
        path = ''
      for (i; i < l; i++) {
        path = (typeof args[i] !== 'function' ? args[i] : args[i]()) + ''
     ...
     **/
    function copy () {      
      return pick(Object.assign({}, data))
    }
     ...
     **/
    function get (...args) {
      if (!args.length) {log('get 方法需要传入对应的子属性的键'); return}
      let i = 0,
        l = args.length,
        temp = data,
        path = '',
    ...
    function set (...args) {
      ...
      path = (typeof args[i] !== 'function' ? args[i] : args[i]()) + ''
      value = typeof args[i + 1] !== 'function' ? args[i + 1] : args[i + 1]()
      temp[path] = value
      return this
    }
